...
function initDatabase() {
  ...
  for (const col of ['body', 'attachments']) {
    const exists = db.prepare('PRAGMA table_info(logs)').all().some(c => c.name === col);
    if (!exists) db.exec(`ALTER TABLE logs ADD COLUMN ${col} TEXT`);
  }
...
function addLog(entry) {
  const body = entry.body ? entry.body.substring(0, 65536) : null;
  const attachments = entry.attachments ? JSON.stringify(entry.attachments) : null;
  const stmt = getDb().prepare(
    'INSERT INTO logs (sender, recipient, subject, bot_id, chat_id, rule_id, status, error, body, attachments) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  );
  stmt.run(entry.sender || null, entry.recipient || null, entry.subject || null,
    ...
    entry.status, entry.error || null, body, attachments);
}

function getLog(id) {
  return getDb().prepare('SELECT * FROM logs WHERE id = ?').get(id);
}
...
function loadSettings() {
  return getDb().prepare('SELECT key, value FROM app_settings').all();
}
